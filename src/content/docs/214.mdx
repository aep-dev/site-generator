...
1.  APIs wishing to convey an expiration <b class="font-extrabold	text-red-700">must</b> rely on a
...
3.  APIs <b class="font-extrabold	text-red-700">must</b> always return the expiration time in the `expire_time` field
...
4.  APIs that rely on the specific semantics of a "time to live" (e.g., DNS
    which must represent the TTL as an integer) <b class="font-extrabold text-green-700">may</b> use an `int64 ttl`
    field (and <b class="font-extrabold	text-yellow-700">should</b> provide an [aep.dev/not-precedent](./0200.md) comment
    in this case).
