...
<b class="text-red-700">must</b> include the unit of measurement as the suffix. When appropriate and
unambiguous, units <b class="text-yellow-700">should</b> use generally accepted abbreviations (for
...
cluster), then the field <b class="text-yellow-700">should</b> use the suffix `_count` (**not** the prefix
`num_`):
...
<Aside type="note" title="Note">
  Fields <b class="text-red-700">must not</b> use unsigned integer types, because many
...
APIs <b class="text-green-700">may</b> create structs to represent quantities when appropriate. When
using these structs as fields, APIs <b class="text-yellow-700">should</b> use the name of the struct as
