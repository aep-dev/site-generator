...
While generic fields are generally rare, a API <b class="text-green-700">may</b> introduce generic field
where necessary. There are several approaches to this depending on how generic
the field needs to be; in general, APIs <b class="text-yellow-700">should</b> attempt to introduce the
"least generic" approach that is able to satisfy the use case.

For example, an API <b class="text-yellow-700">should not</b> use a completely generic field (such as
`google.protobuf.Struct` in protobuf APIs) when the value of the field must
correspond to one of a known number of schemas. Instead, the API <b class="text-yellow-700">should</b> use
...
A `oneof` <b class="text-green-700">may</b> be used to introduce a type union: the user or API is able to
specify one of the fields inside the `oneof`. Additionally, a `oneof` <b class="text-green-700">may</b>
...
each option, and APIs <b class="text-yellow-700">should</b> generally prefer them over other generic or
...
Maps <b class="text-green-700">may</b> be used in situations where many values _of the same type_ are
...
The [`google.protobuf.Struct`][struct] object <b class="text-green-700">may</b> be used to represent
...
If a API needs to reason about the _schema_ of a `Struct`, it <b class="text-yellow-700">should</b> use
...
Because of this, `Any` <b class="text-yellow-700">should not</b> be used unless other options are
infeasible.
