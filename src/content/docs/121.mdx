...
A resource-oriented API <b class="text-red-700">must</b> be modeled as a resource hierarchy, where each
...
<Aside type="note" title="Note">
  ...
  designers <b class="text-yellow-700">should not</b> expect that their API will be reflective of their
...
resource schema for that resource across all methods <b class="text-red-700">must</b> be the same.
...
A resource <b class="text-red-700">must</b> support at minimum [Get][]: clients must be able to
validate the state of resources after performing a mutation such as [Create][],
[Update][], or [Delete][].

A resource <b class="text-red-700">must</b> also support [List][], except for [singleton resources][]
...
APIs <b class="text-yellow-700">should</b> prefer standard methods over custom methods; the purpose of
...
[long-running][long-running-requests] or synchronous) <b class="text-red-700">must</b> mean that the
...
<b class="text-yellow-700">should</b> also have reached a steady-state.

Examples include:

- Following a successful create that is is latest mutation on a resource, a get
  request for a resource <b class="text-red-700">must</b> return the resource.
- Following a successful update that is the latest mutation on a resource, a
  get request for a resource <b class="text-red-700">must</b> return the final values from the update
  request.
- Following a successful delete that is the latest mutation on a resource, a
  get request for a resource <b class="text-red-700">must</b> return `NOT_FOUND` (or the resource with
...
references][], <b class="text-red-700">must</b> be representable via a [directed acyclic graph][].
