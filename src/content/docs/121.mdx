...
state: approved
slug: resources
created: 2024-01-27T00:00:00.000Z
placement:
  category: resources
...
A resource-oriented API <b class="font-extrabold	text-red-700">must</b> be modeled as a resource hierarchy, where each
...
<Aside type="note" title="Note">
  While there is some conceptual alignment between storage systems and
  ...
  designers <b class="font-extrabold	text-yellow-700">should not</b> expect that their API will be reflective of their
...
resource schema for that resource across all methods <b class="font-extrabold	text-red-700">must</b> be the same.
...
A resource <b class="font-extrabold	text-red-700">must</b> support at minimum [Get][]: clients must be able to
validate the state of resources after performing a mutation such as [Create][],
[Update][], or [Delete][].

A resource <b class="font-extrabold	text-red-700">must</b> also support [List][], except for [singleton resources][]
where more than one resource is not possible.

<Aside type="note" title="Note">
  A custom method in resource-oriented design does _not_ entail
...
APIs <b class="font-extrabold	text-yellow-700">should</b> prefer standard methods over custom methods; the purpose of
...
[long-running][long-running-requests] or synchronous) <b class="font-extrabold	text-red-700">must</b> mean that the
...
<b class="font-extrabold	text-yellow-700">should</b> also have reached a steady-state.

Examples include:

- Following a successful create that is is latest mutation on a resource, a get
  request for a resource <b class="font-extrabold	text-red-700">must</b> return the resource.
- Following a successful update that is the latest mutation on a resource, a
  get request for a resource <b class="font-extrabold	text-red-700">must</b> return the final values from the update
  request.
- Following a successful delete that is the latest mutation on a resource, a
  get request for a resource <b class="font-extrabold	text-red-700">must</b> return `NOT_FOUND` (or the resource with
...
references][], <b class="font-extrabold	text-red-700">must</b> be representable via a [directed acyclic graph][].
