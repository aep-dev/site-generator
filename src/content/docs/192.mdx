...
In APIs defined in protocol buffers, public comments <b class="text-red-700">must</b> be included over
...
Services, in particular, <b class="text-yellow-700">should</b> have descriptive comments that explain what
the service is and what users are able to do with it.

<Aside type="note" title="Note">
  Many readers will not be native English speakers. Comments <b class="text-yellow-700">should</b>
  ...
  comments <b class="text-yellow-700">should</b> use people who are non-controversial and no longer alive.
</Aside>

### Style

Comments <b class="text-yellow-700">should</b> be in grammatically correct American English. However, the
first sentence of each comment <b class="text-yellow-700">should</b> omit the subject and be in the
...
Descriptions of messages and fields <b class="text-yellow-700">should</b> be brief but complete. Sometimes
...
Any formatting in comments <b class="text-red-700">must</b> be in [CommonMark][]. Headings and tables
<b class="text-red-700">must not</b> be used, as these cause problems for several tools, and are
unsuitable for client library reference documentation.

Comments <b class="text-yellow-700">should</b> use `code font` for field or method names and for literals
(such as `true`).

Raw HTML <b class="text-red-700">must not</b> be used.

"ASCII art" attempts to present a diagram within the protos <b class="text-red-700">must not</b> be
...
Comments <b class="text-green-700">may</b> "link" to another component (service, method, message, field,
...
Comments <b class="text-green-700">may</b> link to external pages to provide background information
beyond what is described in the public comments themselves. External links
<b class="text-red-700">must</b> use absolute (rather than relative) URLs, including the protocol
(usually `https`), and <b class="text-yellow-700">should not</b> assume the documentation is located on
...
comments, acronyms <b class="text-yellow-700">should not</b> be used, unless the acronym is such dominant
colloquial use that avoiding it would obscure the reference (example: [IBM][]).

Comments <b class="text-yellow-700">should</b> spell and capitalize trademarked names consistent with the
...
<b class="text-red-700">must</b> be set to `true`, and the first line of the respective comment
<b class="text-red-700">must</b> start with `"Deprecated: "` and provide alternative solutions for
developers. If there is no alternative solution, a deprecation reason <b class="text-red-700">must</b>
be given.
...
Comments <b class="text-green-700">may</b> be explicitly marked as internal by wrapping internal content
in `(--` and `--)`.

Non-public links, internal implementation notes (such as `TODO` and `FIXME`
directives), and other such material <b class="text-red-700">must</b> be marked as internal.

<Aside type="note" title="Note">
  Comments <b class="text-yellow-700">should</b> use only leading comments (not trailing comments
  or detached comments). In particular, comments <b class="text-red-700">must not</b> use both a leading
