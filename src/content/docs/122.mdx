...
<Aside type="note" title="Note">
  APIs wishing to do this **must** follow this format consistently
...
<Aside type="note" title="Note">
  The full resource path **should not** be used for cross-API
...
<Aside type="note" title="Note">
  Fields **must not** be called `path` except for this purpose. For
...
<Aside type="note" title="Note">
  Fields **should not** be called `parent` except for this purpose. For
